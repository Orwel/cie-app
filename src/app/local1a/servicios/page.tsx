...
export default function Local1AServiciosPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        // Obtener unit_id para LOCAL_1A
        const unitResponse = await fetch('/api/servicios/units?code=LOCAL_1A');
        const unitData = await unitResponse.json();
        if (!unitData.success || !unitData.data) {
          throw new Error('No se pudo obtener la unidad');
        }
        setUnitId(unitData.data.id);

        // Obtener meter_groups asociados a LOCAL_1A
    ...
    <main className="min-h-screen bg-black">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-6 sm:py-8 md:py-12">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-6 sm:mb-8">
          Servicios Públicos - Local 1A
        </h1>

        {/* Sección: Subir lecturas del mes actual */}
        ...
        {/* Sección: Historial de consumo y pagos */}
        <section>
          <h2 className="text-xl sm:text-2xl font-semibold text-white mb-4 sm:mb-6">
            📅 Historial
          </h2>
